...
// semáforo
sem_t mutex; // protege o readcount
sem_t wrt; // protege o recurso compartilhado
...
int main(int argc, char ** argv) {
    ...
    // inicializa o semáforo do leitor
    sem_init(&mutex, 0, 1);
    // inicializa o semáforo do escritor
    sem_init(&wrt, 0, 1);
...
void * leitor(void * id) {
    ...
    sem_wait(&mutex);
    readcount++;
    if(readcount == 1){ // primeiro leitor
        sem_wait(&wrt); // o escritor será bloqueado
    }
    sem_post(&mutex);
    ...
    sem_wait(&mutex);
    readcount--;
    if(readcount == 0){ // último leitor
        sem_post(&wrt); // libera o escritor
    }
    sem_post(&mutex);
