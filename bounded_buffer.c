...
//
sem_t mutex; // garante a exclusão mútua ao acessar o buffer (seção crítica)
sem_t vazio; // diz quantas posições vazias tem no buffer 
sem_t cheio; // diz quantas posições cheias tem no buffer
...
int main(int argc, char ** argv){
    ...
    // valores)
    //
    sem_init(&mutex, 0, 1); // disponível
    sem_init(&vazio, 0, N_BUFFER); // todo o buffer esta vazio
    sem_init(&cheio, 0, 0); // nada pronto
